...
def test_import_generate_api() -> None:
    ...
    imported = client.post("/api/schemas/import", data={"yaml": SAMPLE_SCHEMA}, content_type="application/json")
...
def test_integration_session_yaml_api() -> None:
    ...
    updated = client.put(
        f"/api/integrations/sessions/{body['session_id']}/tables",
        data={"tables": tables},
        content_type="application/json",
    )
...
def test_stale_vite_asset_hash_resolves_to_current_chunk(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(views, "FRONTEND_ASSETS", assets)

    assert views._resolve_frontend_asset("jquery-oldhash.js") == current_jquery
    assert views._resolve_frontend_asset("missing-image-oldhash.png") is None
